...
import SimpleInstrument from '@/components/instruments/instruments/SimpleInstrument';
...
const MusicRoom = () => {
  ...
  useEffect(() => {
    const messageHandler = (e: StorageEvent) => {
      if (e.key === `room_messages_${roomId}`) {
  ...
  useEffect(() => {
    ...
    if (room) {
      ...
      // Check if current user is the admin (host)
      const hostParticipant = room.participants.find((p: any) => p.isHost);
      
      // Check if we're reconnecting as host
      const isLocalHost = localStorage.getItem(`room_host_${roomId}`) === 'true';
      
      if (isLocalHost) {
        // User is returning as admin/host - auto join them
        setIsAdmin(true);
        setUserName('Room Admin');
        setShowNameInput(false);
      } else {
        // Regular user joining - only needs to enter name
    ...
    // Load any existing messages for this room
    const existingMessages = JSON.parse(localStorage.getItem(`room_messages_${roomId}`) || '[]');
  ...
  const addMessage = (message: Partial<Message>) => {
    ...
    setMessages(prev => {
      ...
      // Store messages in localStorage to maintain between refreshes
      localStorage.setItem(`room_messages_${roomId}`, JSON.stringify(updatedMessages));
      // Dispatch storage event to notify other tabs
      window.dispatchEvent(new StorageEvent('storage', {
        key: `room_messages_${roomId}`,
  ...
  const destroyRoom = () => {
    ...
    localStorage.setItem('musicRooms', JSON.stringify(updatedRooms));
    localStorage.removeItem(`room_messages_${roomId}`);
    localStorage.removeItem(`room_host_${roomId}`);
    
    toast({
      title: "Room closed",
      description: "The music room has been closed",
    });
  ...
  const handleJoinRoom = () => {
    ...
    setTimeout(() => {
      const newUser = {
        ...
        name: userName,
        // Use the default instrument or let user choose if different instruments allowed
        instrument: roomSettings.allowDifferentInstruments ? userInstrument || roomSettings.hostInstrument : roomSettings.hostInstrument,
